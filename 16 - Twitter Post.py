...
query_codificada = urllib.parse.quote(query, safe='') #para conseguir mandar caracteres especiais
requisicao = cliente.request('https://api.twitter.com/1.1/statuses/update.json?status=' + query_codificada, method='POST')

decodificar = requisicao[1].decode() #para transformar a resposta Bytes em algo legível
